...
final class UnusedVariableLinterTest extends TestCase {
  ...
  public function getCleanExamples(): vec<(string)> {
    return vec[
      ...
      tuple(
        '<?hh function foo() { $users = dict[]; $u = dict["id" => 15]; $users[$u["id"]] = 5; return $users; }',
      ),
      tuple('<?hh function foo() { $s = "a"; echo "{$s}"; }'),
      tuple(
        '<?hh function foo() { return (inout vec<int> $p) ==> { $p[] = 1; }; }',
      ),
      tuple(
        '<?hh function foo() { return function(inout vec<int> $p) ==> { $p[] = 1; }; }',
      ),
      ...
      tuple(
        '<?hh class C { public function foo() { $d = dict[]; return $d[1]; } }',
      ),
      tuple(
        '<?hh class C { public function foo() { $d = dict[]; $d["a"] = "b"; f($d["a"]); } }',
      ),
      tuple('<?hh class C { public function foo(inout $d) { $d = 5; } }'),
      tuple(
        '<?hh class C { public function foo() { $users = dict[]; $u = dict["id" => 15]; $users[$u["id"]] = 5; return $users; } }',
      ),
      tuple(
        '<?hh class C { public function foo() { $s = "a"; echo "{$s}"; } }',
      ),
      tuple(
        '<?hh class C { public function foo(string $x = $this->x) { echo $x; } }',
      ),
      tuple(
        '<?hh class C { public int $x = 1; public function foo() { $this->x = 3; } }',
      ),
      tuple(<<<HACK
<?hh
class C {
