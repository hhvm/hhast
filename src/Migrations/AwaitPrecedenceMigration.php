...
use function Facebook\HHAST\from_file_args;
...
final class AwaitPrecedenceMigration extends BaseMigration {
    ...
    EditableNode $ast,
      ...
      $nodes = Vec\filter($nodes, $n ==> $n->getOperator()->getCode() === 'await');
      if(C\is_empty($nodes)){ return $ast; }
      
      // Figure out how this parses with stronger await precedence.
      //  If this throws a parse error, just let the caller deal with it.
      $stronger_await_ast = from_file_args($path, vec['--stronger-await-binding']);
      $stronger_nodes = $stronger_await_ast->getDescendantsOfType(PrefixUnaryExpression::class);
      $stronger_nodes = Vec\filter($nodes, $n ==> $n->getOperator()->getCode() === 'await');
          ...
          if($nodes[$i]->getOperand()->getCode() !== 
            $stronger_nodes[$i]->getOperand()->getCode()){
                ...
                $ast = $ast->replace($target, new ParenthesizedExpression(
                    ...
                    new RightParenToken(Missing(), $target->getLastTokenx()->getTrailing())
                ));
          }
      }
