  ...
  public function getFixedNode(
    ...
    if (!C\any($sorted_clauses, $c ==> Str\contains($c->getCode(), "\n"))) {
      ...
      $clauses = Vec\map(
        ...
        $clause ==> {
          if ($clause === $last) {
            return $clause->withSeparator(Missing());
          }
          return $clause->hasSeparator()
            ? $clause
            : $clause->withSeparator(
              new CommaToken(Missing(), new NodeList(vec[new WhiteSpace(' ')])),
            );
        },
      );
    } else {
      ...
      $clauses = Vec\map(
        ...
        $clause ==> {
          ...
          return $clause->replace($t, $t->withTrailing(Missing()))
            ->withSeparator(new CommaToken(Missing(), $trailing));
        },
      );
    }
